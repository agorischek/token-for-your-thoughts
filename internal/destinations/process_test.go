...
func TestProcessDestinationSubmit(t *testing.T) {
	t.Setenv("GO_WANT_HELPER_PROCESS", "1")

	destination, err := NewProcessDestination(config.DestinationConfig{
		Type:    "process",
		Command: os.Args[0],
		Args:    []string{"-test.run=TestHelperProcessProcessDestination", "--", "ok"},
		Method:  "submit_feedback",
	})
	if err != nil {
		t.Fatalf("new process destination: %v", err)
	}

	item, err := feedback.New("Claude Code", "The process destination should deliver JSON-RPC over stdio.", "cli", map[string]any{"tool": "test"})
	if err != nil {
...
func TestProcessDestinationSubmitJSONRPCError(t *testing.T) {
	t.Setenv("GO_WANT_HELPER_PROCESS", "1")

	destination, err := NewProcessDestination(config.DestinationConfig{
		Type:    "process",
		Command: os.Args[0],
		Args:    []string{"-test.run=TestHelperProcessProcessDestination", "--", "error"},
		Method:  "submit_feedback",
	})
	if err != nil {
		t.Fatalf("new process destination: %v", err)
...
func TestProcessDestinationReusesProcess(t *testing.T) {
	t.Setenv("GO_WANT_HELPER_PROCESS", "1")
...
func TestHelperProcessProcessDestination(t *testing.T) {
