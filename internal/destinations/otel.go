...
func NewOTelDestination(ctx context.Context, cfg config.DestinationConfig) (*OTelDestination, error) {
	...
	res, err := resource.New(ctx, resource.WithAttributes(
		...
		attribute.String("tfyt.destination", "otel"),
	))
	if err != nil {
...
func (s *OTelDestination) Name() string {
	return s.name
}

func (s *OTelDestination) Close(ctx context.Context) error {
	return s.provider.Shutdown(ctx)
}

func (s *OTelDestination) Submit(ctx context.Context, item feedback.Item) error {
